...
func (kq *KeysQuery) loadHost(ctx context.Context, query *HostsQuery, nodes []*Keys, init func(*Keys), assign func(*Keys, *Hosts)) error {
	...
	for _, n := range neighbors {
		fk := n.keys_host
		if fk == nil {
			return fmt.Errorf(`foreign-key "keys_host" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "keys_host" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
